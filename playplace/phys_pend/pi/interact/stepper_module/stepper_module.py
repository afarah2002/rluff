...
class StepperMotor(object):
	...
	def __init__(self, GPIO_pins, drive="half", CONT_BOOL=False, init_delay=0.05):
		self.motor = RpiMotorLib.BYJMotor("stepper_motor", "Nema")
	...
	def turn_to_angle(self, speed_pct, angle=None, d_angle=None):
		...
		steps = abs(angle_diff)*200./360.

		# Delay (half vs full)
		min_delay = {"half":0.0009, 
					 "full":0.0013}

		# delay = 0.0005*self.drive_conversions[self.drive]/(speed_pct/100)
		delay = 0.01*self.drive_conversions[self.drive]/(speed_pct/100)
		print(delay)
	...
	def single_turn_test(self, step):
		self.motor.motor_run(self.GPIO_pins, 0.0005, step, False, False, "half", .05)
		print(step)

if __name__ == '__main__':
	GPIO_pins = [17,27,22,23]
	stroke_plane_servo = StepperMotor(GPIO_pins, drive="full", CONT_BOOL=False)
	# speed_pct = 100
	# angle = 90
	# d_angle = 90
	# while True:
	...
	try:
		...
		while True:
			...
			stroke_plane_servo.turn_to_angle(speed_pct, d_angle=d_angle)
			# step += 100
			# stroke_plane_servo.single_turn_test(step)
