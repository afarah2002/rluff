...
def custom_plotter(input_dir):
	times = list(read_raw_data(input_dir, "Wing torques", "X"))
	torques = list(read_raw_data(input_dir, "Wing torques", "Y")[:,0])
	raw_angles = list(read_raw_data(input_dir, "Wing angles", "Y")[:,0])
	ang_vels = list(read_raw_data(input_dir,"Angular velocity", "Y")[:,0])
