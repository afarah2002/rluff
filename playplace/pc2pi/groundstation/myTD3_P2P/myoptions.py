	...
	def brute_force(self, out_q):
		...
		for t in range(int(self.args.max_timesteps)):
			...
			for time_i in range(len(t_0)):
				...
				data_pack = {"joint torques" : [t_0[time_i], list(action)],
							 "stroke plane" : [episode_num, [episode_reward]]}
				out_q.put(data_pack)
				# done_bool = float(done) if episode_timesteps < self.env._max_episode_steps else 0
				done = False	
				if time_i != len(t_0) - 1:
	...
	def consumer(self, 
		...
		while True:
			for new_data in iter(in_q.get, sentinel):
				...
				data_pack = {"joint torques": [data_storage.XData[-1], 
											   data_storage.actions_data[-1]],
							 "stroke plane" : [data_storage.episode_numbers[-1], 
							 				   data_storage.episodic_reward_data[-1]]} 
				
				data_pack = {"joint torques": [new_data[0], 
											   new_data[1]],
							 "stroke plane" : [new_data[2], 
							 				  [new_data[3]]]} 


				gui_server.send_data_pack(data_pack)


	def gs_server_queue(self,
						queue,
						pc_server):
		...
		while True:
			new_data = queue.get()
			if new_data:
				self.send_to_pi(new_data)
				pass
				# send to pi
	def send_to_pi(self, new_data):
		self.pc_server.send_data_pack(new_data)
		pass
