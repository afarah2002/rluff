...
def eval_policy(policy, env_name, seed, env_action_dim, no_terms, eval_episodes=10,):
	eval_env = gym.make(env_name)
	eval_env.seed(seed + 100)
	...
	for _ in range(eval_episodes):
		...
		while not done:
			...
			new_amps = primes[:int(len(primes)/2)]
...
class Options(object):
	...
	def __init__(self):
		...
		self.parser.add_argument("--env", default="HalfCheetah-v2")          # OpenAI gym environment name
	...
	def brute_force(self, out_q):
		...
		no_terms = 4
		self.action_dim = self.env_action_dim * 2 * no_terms
		...
		# Evaluate untrained policy
		self.evaluations = [eval_policy(self.policy, self.args.env, self.args.seed, self.env_action_dim, no_terms)]
		self.replay_buffer = myutils.ReplayBuffer(self.state_dim, self.action_dim)
		...
		for t in range(int(self.args.max_timesteps)):
			...
			# Select action randomly or according to policy
			if t < self.args.start_timesteps:
				new_amps = np.array([random.uniform(-1.,1) for _ in range(self.env_action_dim*no_terms)])
				# print(new_amps)
				new_freqs = np.array([random.uniform(f_bounds[0], f_bounds[1]) for _ in range(self.env_action_dim*no_terms)])
			...
			# print(primes)
			# Store data in replay buffer
			self.replay_buffer.add(state, primes, next_state, reward, done_bool)
			...
			# Evaluate episode
			if (t + 1) % self.args.eval_freq == 0:
				self.evaluations.append(eval_policy(self.policy, self.args.env, self.args.seed, self.env_action_dim, no_terms))
				np.save(f"./results/{self.file_name}", self.evaluations)
				if self.args.save_model: self.policy.save(f"./models/{self.file_name}")
