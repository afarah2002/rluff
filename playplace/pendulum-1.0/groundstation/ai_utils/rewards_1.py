...
class R_omega:
	...
	def polyn_1(omega, omega_target, A=100, B=50, C=100):
		# Quadratic
		return -A*((np.abs(omega) - omega_target)/B)**2 + C
...
class R_theta:
	...
	def polyn_1(theta, A=1, B=40, C=10):
		# -A(x/B)^4 + C
		return -A*(theta/B)**6 + C
...
class Rewards_1(object):
	...
	def reward_2(self, combo_data, target):
		...
		# Update copies of data class subsets
		prev_wing_trq = np.append(prev_wing_trq, wing_torque[0])
		...
		# Calculate reward
		# R_ang_vel = 10*R_omega.hyperb_1(ang_vel, target_ang_vel)
		R_ang_vel = 20.*R_omega.polyn_1(ang_vel, target_ang_vel)
		# R_ang_vel = 0.1*R_omega.log_1(ang_vel, target_ang_vel)
		# Did we hit the failsafe? Do not reward this! LOL
