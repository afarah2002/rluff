...
def read_raw_data(test_num, # Name of dir in test_data/
				  ...
				  axis):	# X or Y
	file_name = f"test_data_2/{test_num}/{fig_name}/{axis.capitalize()}Data.txt" 
...
def delay_analysis(data_dir):
	timesteps = np.array(read_raw_data(data_dir, "Real time", "X"))
	real_times = np.array(read_raw_data(data_dir, "Real time", "Y")).flatten()

	dts = np.diff(real_times)

	plt.hist(dts,bins=int(np.size(dts)/50))

	dt_avg = np.mean(dts)
	print(f"Average dt: {dt_avg}")
